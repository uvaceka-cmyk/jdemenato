...
export async function POST(request: Request) {
  ...
  const { success } = await env.REQUEST_LIMITER.limit({ key: `poptavka:${ip}` });
  if (!success) return Response.json({ error: "Příliš mnoho poptávek z vaší sítě. Zkuste to prosím za chvíli." }, { status: 429 });
  const body = await request.json() as Record<string, unknown>;
  if (typeof body.website === "string" && body.website.trim()) return Response.json({ error: "Poptávku se nepodařilo uložit." }, { status: 400 });
