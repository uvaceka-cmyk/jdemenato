...
export default async function Page({params}:{params:Promise<{slug:string}>}){const{slug}=await params;const c=categories.find(x=>slugify(x[0])===slug);if(!c)notFound();return <main className="simple"><Link href="/" className="brand"><span>JdemNa</span><strong>To!</strong></Link><p className="eyebrow">OBOR</p><h1>{c[0]}</h1><p>{c[1]}</p><h2>Aktuální nabídky</h2><div className="job-grid">{jobs.map(j=><Link className="job-card" href={`/prace/${j.slug}`} key={j.slug}><small>{j.company}</small><h3>{j.title}</h3><b>{j.place} · {j.pay}</b></Link>)}</div></main>}
